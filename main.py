...
def parse_arguments():
    ...
    parser.add_argument(
        "--random_seed", type=int, default=None, help="Random seed for reproducibility"
    )
    parser.add_argument(
        "--cache_enabled",
        action="store_true",
...
def main(args):
    ...
    glog.info(
        f"Time cost for generating scene graph: {time.perf_counter() - t:.2f} seconds"
    )
    visualization_tools.quick_visualize_scene(
        scene_graph, os.path.join(main_config.output_dir, "scene_graph.dot")
    )
    ...
    atomic_task = None
    if main_config.atomic_task_pkl_load_path is not None and os.path.exists(
        main_config.atomic_task_pkl_load_path
    ):
    ...
    for i, task in enumerate(atomic_task.tasks):
        print(
            f"Task {i}: {task.__repr_rough__()}",
            file=open(os.path.join(main_config.output_dir, "tasks.txt"), "a"),
        )
    ...
    for i in task_list:
        ...
        with open(main_config.result_file_path, "a") as f:
            f.write(
                f"Task {i}: {task.status}, Task Type: level {task_sample[i].is_ambiguous(scene_graph) + 1}, type {task_sample[i].type}, Score: {task.partial_score}\n"
            )
